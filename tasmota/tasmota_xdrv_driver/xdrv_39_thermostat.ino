...
#define D_CMND_RADIATORTEMPMEASUREDSET "RadiatorTempMeasuredSet"
...
#define D_CMND_THERMOREAD		"ThermoRead"
#define D_CMND_RATEMPMAXSET 	"RaTempMaxSet"
#define D_CMND_RATEMPMINSET 	"RaTempMinSet"


enum ThermostatModes
{
	THERMOSTAT_OFF, THERMOSTAT_AUTOMATIC_OP, THERMOSTAT_MANUAL_OP, THERMOSTAT_MODES_MAX
};
...
{
	CTR_HYBRID, CTR_PI, CTR_RAMP_UP, CTR_PI_AUTOTUNE, CTR_MODES_MAX
};


enum ControllerHybridPhases
{
	CTR_HYBRID_RAMP_UP, CTR_HYBRID_PI, CTR_HYBRID_PI_AUTOTUNE
};


enum AutotuneStates
{
	AUTOTUNE_OFF, AUTOTUNE_ON, AUTOTUNE_MAX
};


enum AutotunePerformanceParam
{
	AUTOTUNE_PERF_FAST, AUTOTUNE_PERF_NORMAL, AUTOTUNE_PERF_SLOW, AUTOTUNE_PERF_MAX
};
...
enum ControllerModes
{
	CTR_HYBRID, CTR_PI, CTR_RAMP_UP, CTR_MODES_MAX
};
...
enum ClimateModes
{
	CLIMATE_HEATING, CLIMATE_COOLING, CLIMATE_MODES_MAX
};
...
enum SensorType
{
	SENSOR_MQTT, SENSOR_LOCAL, SENSOR_MAX
};
...
enum TempConvType
{
	TEMP_CONV_ABSOLUTE, TEMP_CONV_RELATIVE
};


enum DiagnosticModes
{
	DIAGNOSTIC_OFF, DIAGNOSTIC_ON
};
...
typedef union 
{
	...
	struct 
		...
		uint32_t		ra_sensor_alive : 1;		// Flag stating if radiator temperature sensor is alive (0 = inactive, 1 = active)
...
uint16_t Domoticz_Virtual_Switches[DOMOTICZ_MAX_IDX] =
{
	DOMOTICZ_IDX1, DOMOTICZ_IDX3, DOMOTICZ_IDX4, DOMOTICZ_IDX5
};


#endif // DEBUG_THERMOSTAT

const char kThermostatCommands[] PROGMEM = "|" D_CMND_THERMOSTATMODESET "|" D_CMND_CLIMATEMODESET "|" D_CMND_TEMPFROSTPROTECTSET "|" D_CMND_CONTROLLERMODESET "|" D_CMND_INPUTSWITCHSET "|" D_CMND_INPUTSWITCHUSE "|" D_CMND_OUTPUTRELAYSET "|" D_CMND_TIMEALLOWRAMPUPSET "|" D_CMND_TEMPFORMATSET "|" D_CMND_TEMPMEASUREDSET "|" D_CMND_RADIATORTEMPMEASUREDSET "|" D_CMND_TEMPTARGETSET "|" D_CMND_TEMPMEASUREDGRDREAD "|" D_CMND_SENSORINPUTSET "|" D_CMND_STATEEMERGENCYSET "|" D_CMND_TIMEMANUALTOAUTOSET "|" D_CMND_PROPBANDSET "|" D_CMND_TIMERESETSET "|" D_CMND_TIMEPICYCLESET "|"
...
D_CMND_TIMEMINACTIONSET "|" D_CMND_TIMEMINTURNOFFACTIONSET "|" D_CMND_TEMPRUPDELTINSET "|" D_CMND_TEMPRUPDELTOUTSET "|" D_CMND_TIMERAMPUPMAXSET "|" D_CMND_TIMERAMPUPCYCLESET "|" D_CMND_TEMPRAMPUPPIACCERRSET "|" D_CMND_TIMEPIPROPORTREAD "|" D_CMND_TIMEPIINTEGRREAD "|" D_CMND_TIMESENSLOSTSET "|" D_CMND_DIAGNOSTICMODESET "|" D_CMND_CTRDUTYCYCLEREAD "|" D_CMND_ENABLEOUTPUTSET "|" D_CMND_THERMOREAD "|" D_CMND_RATEMPMAXSET "|" D_CMND_RATEMPMINSET;
void(*const ThermostatCommand[]) (void) PROGMEM =
{
	&CmndThermostatModeSet, &CmndClimateModeSet, &CmndTempFrostProtectSet, &CmndControllerModeSet, 
	&CmndInputSwitchSet, 
	&CmndInputSwitchUse, &CmndOutputRelaySet, &CmndTimeAllowRampupSet, &CmndTempFormatSet, &CmndTempMeasuredSet, &CmndRadiatorTempMeasuredSet, &CmndTempTargetSet, &CmndTempMeasuredGrdRead, 
	&CmndSensorInputSet, &CmndStateEmergencySet, 
	&CmndTimeManualToAutoSet, 
	&CmndPropBandSet, &CmndTimeResetSet, &CmndTimePiCycleSet, &CmndTempAntiWindupResetSet, &CmndTempHystSet, 

#ifdef USE_PI_AUTOTUNING
	& CmndPerfLevelAutotune, &CmndTimeMaxActionSet, &CmndTimeMinActionSet, &CmndTimeMinTurnoffActionSet, 
	&CmndTempRupDeltInSet, 
...
struct THERMOSTAT
{
	...
	int32_t time_integral_pi = 240; 				// Time integral part of the PI controller
	int32_t time_integral_pi_previous = 400;
	...
	int16_t temp_measured = 220;					// Temperature measurement received from sensor in tenths of degrees celsius
	int16_t temp_measured_accum_error = 0;			// Temperature measurement received from sensor in tenths of degrees celsius
	int16_t temp_measured_accum_count = 0;
	int16_t radiator_temp_measured = 0;
	int16_t radiator_temp_max = 400;
	int16_t radiator_temp_max_setting = 520;
	int16_t radiator_temp_min_setting = 360;
	uint32_t		timestamp_radiator_temp_measured_update;
...
bool ThermostatHalfMinuteCounter(uint8_t ctr_output)
{
	bool result = false;

	Thermostat[ctr_output].status.counter_seconds++; // increment time
	if ((Thermostat[ctr_output].status.counter_seconds % 20) == 0) //change to every 20 seconds
		{
		result = true;
		Thermostat[ctr_output].status.counter_seconds = 0;
		}
...
void ThermostatSignalPreProcessingSlow(uint8_t ctr_output)
{
	...
	if ((TasmotaGlobal.uptime - Thermostat[ctr_output].timestamp_radiator_temp_measured_update) > ((uint32_t) Thermostat[ctr_output].time_sens_lost * 60))
		{
		Thermostat[ctr_output].status.ra_sensor_alive = IFACE_OFF;
		Thermostat[ctr_output].radiator_temp_measured = 600;
		}
}
...
void ThermostatSignalPostProcessingSlow(uint8_t ctr_output)
{
	...
	if ((Thermostat[ctr_output].status.status_output != Thermostat[ctr_output].status.command_output) && (Thermostat[ctr_output].status.enable_output == IFACE_ON))
		{
...
void ThermostatCtrState(uint8_t ctr_output)
{
	...
	switch (Thermostat[ctr_output].status.controller_mode)
		...
		case CTR_PI:
			...
			if ((Thermostat[ctr_output].status.autotune_flag == AUTOTUNE_ON) && (Thermostat[ctr_output].temp_measured == Thermostat[ctr_output].temp_target_level) && ((flag_heating &&
				 (Thermostat[ctr_output].temp_measured_gradient < 0)) || (!flag_heating && (Thermostat[ctr_output].temp_measured_gradient > 0))))
				{
	...
	if (Thermostat[ctr_output].status.controller_mode == CTR_HYBRID)
		...
		switch (Thermostat[ctr_output].status.phase_hybrid_ctr)
			...
			case CTR_HYBRID_RAMP_UP:
				...
				if ((Thermostat[ctr_output].time_ctr_checkpoint != 0) && (TasmotaGlobal.uptime >= Thermostat[ctr_output].time_ctr_checkpoint))
					{
					// Reset pause period
					...
					// Set PI controller
					Thermostat[ctr_output].time_integral_pi_previous = ((uint32_t) Thermostat[ctr_output].time_max_action * 60);
					Thermostat[ctr_output].status.phase_hybrid_ctr = CTR_HYBRID_PI;
					}
			...
			case CTR_HYBRID_PI:
				...
				// then go to ramp-up
				if ((Thermostat[ctr_output].temp_target_level != Thermostat[ctr_output].temp_target_level_ctr) &&
...
{
	bool change_state = false;						// If input is used

	// AND switch input is active
	// OR temperature sensor is not alive
	// then go to manual
	if ((Thermostat[ctr_output].status.use_input == INPUT_USED) && ((Thermostat[ctr_output].status.status_input == IFACE_ON) || (Thermostat[ctr_output].status.sensor_alive == IFACE_OFF)))
		{
		change_state = true;
		}
...
{
	bool change_state = false;						// If switch input inactive

	// AND sensor alive
	// AND no switch input action (time in current state) bigger than a pre-defined time
	// then go to automatic
	if ((Thermostat[ctr_output].status.status_input == IFACE_OFF) && (Thermostat[ctr_output].status.sensor_alive == IFACE_ON) &&
...
void ThermostatState(uint8_t ctr_output)
{
	switch (Thermostat[ctr_output].status.thermostat_mode)
		...
		case THERMOSTAT_OFF: // No change of state possible without external command
			break; // State automatic, thermostat active following the command target temp.
		...
		case THERMOSTAT_AUTOMATIC_OP:
			...
			break; // State manual operation following input switch
...
{ // If command received to enable output
	...
	if ((command == IFACE_ON) && (Thermostat[ctr_output].status.status_output == IFACE_OFF))
...
{ // General comment: Some variables have been increased in resolution to avoid loosing accuracy in division operations
	bool flag_heating = (Thermostat[ctr_output].status.climate_mode == CLIMATE_HEATING);
	int32_t aux_temp_error; 						//temp Target changed 

	if (Thermostat[ctr_output].temp_target_level != Thermostat[ctr_output].temp_target_level_ctr)
		{
		int16_t target_error;

		target_error = Thermostat[ctr_output].temp_target_level - Thermostat[ctr_output].temp_target_level_ctr;
		Thermostat[ctr_output].temp_measured_accum_error = Thermostat[ctr_output].temp_measured_accum_error + target_error * Thermostat[ctr_output].temp_measured_accum_count;
		Thermostat[ctr_output].temp_target_level_ctr = Thermostat[ctr_output].temp_target_level;
		} //if no measurement 

	if (Thermostat[ctr_output].status.sensor_alive == IFACE_OFF)
		{
		return;
		} // Calculate error

	aux_temp_error = (int32_t) (Thermostat[ctr_output].temp_target_level_ctr - Thermostat[ctr_output].temp_measured) * 10; // Invert error for cooling
	...
	Thermostat[ctr_output].kP_pi = 100 / (uint16_t) (Thermostat[ctr_output].val_prop_band); // Calculate proportional
	Thermostat[ctr_output].time_proportional_pi =
		 ((int32_t) (Thermostat[ctr_output].temp_pi_error * (int16_t) Thermostat[ctr_output].kP_pi) * ((int32_t) Thermostat[ctr_output].time_pi_cycle * 6)) / 1000; // Calculate integral
	if (Thermostat[ctr_output].status.status_cycle_active == CYCLE_ON)
		{ //	(resolution increased to avoid use of floats in consequent operations)
		Thermostat[ctr_output].kI_pi = (uint16_t) ((((uint32_t) Thermostat[ctr_output].kP_pi * (uint32_t) Thermostat[ctr_output].time_pi_cycle * 60)) / (uint32_t) Thermostat[ctr_output].time_reset);

		//if there is valid measurement
		if (Thermostat[ctr_output].temp_measured_accum_count > 0)
			{
			aux_temp_error = Thermostat[ctr_output].temp_measured_accum_error * 100 / Thermostat[ctr_output].temp_measured_accum_count;
			Thermostat[ctr_output].temp_measured_accum_error = 0;
			Thermostat[ctr_output].temp_measured_accum_count = 0;
			}
		else 
			{
			aux_temp_error = 0; 					//set the timer to not enougth trigger a cycle. 
			Thermostat[ctr_output].time_integral_pi_previous = ((uint32_t) Thermostat[ctr_output].time_min_action * 60) - 10;
			}

		if (aux_temp_error > Thermostat[ctr_output].temp_reset_anti_windup * 100)
			{
			Thermostat[ctr_output].time_integral_pi = ((int32_t) Thermostat[ctr_output].time_max_action * 60);
			Thermostat[ctr_output].time_integral_pi_previous = Thermostat[ctr_output].time_integral_pi / 2;
		...
		// and current error will be cummulated to the previous one
			...
			Thermostat[ctr_output].time_integral_pi =
				 Thermostat[ctr_output].time_integral_pi_previous + (((int32_t) Thermostat[ctr_output].temp_pi_accum_error * (int32_t) Thermostat[ctr_output].kI_pi) * (int32_t) ((uint32_t) Thermostat[ctr_output].time_pi_cycle * 6)) / 10000;

			// If integral calculation is bigger than maximum cycle time, adjust result
			// to the cycle time and error will not be cummulated

			/*if (Thermostat[ctr_output].time_integral_pi > ((int32_t) Thermostat[ctr_output].time_max_action * 60))
				{
				Thermostat[ctr_output].time_integral_pi = ((int32_t) Thermostat[ctr_output].time_max_action * 60);
				}
			else if (Thermostat[ctr_output].time_integral_pi < ((int32_t) Thermostat[ctr_output].time_min_action * 60))
				{
				Thermostat[ctr_output].time_integral_pi = ((int32_t) Thermostat[ctr_output].time_min_action * 60) - 10;
				}
			Thermostat[ctr_output].time_integral_pi_previous = Thermostat[ctr_output].time_integral_pi;*/
			}
		} // Calculate output

	Thermostat[ctr_output].time_total_pi = Thermostat[ctr_output].time_proportional_pi + Thermostat[ctr_output].time_integral_pi;
	if (Thermostat[ctr_output].time_total_pi < Thermostat[ctr_output].time_min_action * 60)
		{
	...
	// to the cylce time minus safety time and error will not be cummulated
		...
		{
		if (Thermostat[ctr_output].timestamp_output_off > 0)
			{
			uint32_t time_from_lastact = TasmotaGlobal.uptime - Thermostat[ctr_output].timestamp_output_off;

			Thermostat[ctr_output].time_integral_pi_previous = Thermostat[ctr_output].time_total_pi;
			if (time_from_lastact > ((uint32_t) Thermostat[ctr_output].time_pi_cycle * 60))
				{
				Thermostat[ctr_output].time_total_pi += (uint32_t) Thermostat[ctr_output].time_min_action * time_from_lastact / ((uint32_t) Thermostat[ctr_output].time_pi_cycle * 2);
				}
			}

		if (Thermostat[ctr_output].time_total_pi > ((int32_t) Thermostat[ctr_output].time_max_action * 60))
			...
			{
			Thermostat[ctr_output].time_total_pi = (((int32_t) Thermostat[ctr_output].time_pi_cycle * 60) - ((int32_t) Thermostat[ctr_output].time_min_turnoff_action * 60));
			}
		} // Adjust output switch point

	Thermostat[ctr_output].time_ctr_changepoint = TasmotaGlobal.uptime + (uint32_t) Thermostat[ctr_output].time_total_pi; // Adjust next cycle point
	Thermostat[ctr_output].time_ctr_checkpoint = TasmotaGlobal.uptime + ((uint32_t) Thermostat[ctr_output].time_pi_cycle * 60);
}


void ThermostatRadiator(uint8_t ctr_output)
{
	...
	if (Thermostat[ctr_output].status.command_output == IFACE_OFF)
		{
		//if no measurement 
		if ((Thermostat[ctr_output].status.sensor_alive == IFACE_OFF) || (Thermostat[ctr_output].status.ra_sensor_alive == IFACE_OFF))
			{
			return;
			}
		...
		Thermostat[ctr_output].time_proportional_pi = (int32_t) (aux_temp_error * (int16_t) Thermostat[ctr_output].val_prop_band);

		// Calculate integral
		if (Thermostat[ctr_output].time_ctr_checkpoint != 0)
			{
			aux_temp_error = TasmotaGlobal.uptime - Thermostat[ctr_output].time_ctr_checkpoint;
			}
		else 
			{
			aux_temp_error = 0;
		...
		Thermostat[ctr_output].time_integral_pi = -aux_temp_error / (6 * Thermostat[ctr_output].time_min_action);
		if (Thermostat[ctr_output].time_integral_pi < -80)
			Thermostat[ctr_output].time_integral_pi = -80;

		/*else if (Thermostat[ctr_output].time_integral_pi>-20)
			Thermostat[ctr_output].time_integral_pi=0;*/
		

		Thermostat[ctr_output].radiator_temp_max = Thermostat[ctr_output].time_integral_pi_previous + Thermostat[ctr_output].time_integral_pi + Thermostat[ctr_output].time_proportional_pi;
		if (Thermostat[ctr_output].radiator_temp_max > Thermostat[ctr_output].radiator_temp_max_setting)
			{
			Thermostat[ctr_output].radiator_temp_max = Thermostat[ctr_output].radiator_temp_max_setting;
			Thermostat[ctr_output].time_ctr_checktime = Thermostat[ctr_output].time_min_turnoff_action * 0.75;
			}
		else if (Thermostat[ctr_output].radiator_temp_max < Thermostat[ctr_output].radiator_temp_min_setting)
			{
			Thermostat[ctr_output].radiator_temp_max = Thermostat[ctr_output].radiator_temp_min_setting;
			Thermostat[ctr_output].time_ctr_checktime = Thermostat[ctr_output].time_min_turnoff_action * 1.25;
			}
		else 
			Thermostat[ctr_output].time_ctr_checktime = Thermostat[ctr_output].time_min_turnoff_action;

		aux_temp_error = Thermostat[ctr_output].radiator_temp_max - Thermostat[ctr_output].radiator_temp_measured;
		if (aux_temp_error < 70)
			return;

		int time = aux_temp_error * 0.1 + 5;

		if (time > Thermostat[ctr_output].time_max_action)
			time = Thermostat[ctr_output].time_max_action;

		Thermostat[ctr_output].time_integral_pi_previous = Thermostat[ctr_output].radiator_temp_max;
		Thermostat[ctr_output].time_ctr_changepoint = TasmotaGlobal.uptime + time * 60;
		Thermostat[ctr_output].status.command_output = IFACE_ON;
		}
	else 
		{
		//lost radiator mesauremtn 
		if (Thermostat[ctr_output].status.ra_sensor_alive == IFACE_OFF)
			{
			Thermostat[ctr_output].status.command_output = IFACE_OFF;
			return;
			}

		//temp reach maxm or time pass maxim 
		if ((Thermostat[ctr_output].radiator_temp_measured >= Thermostat[ctr_output].radiator_temp_max) || (TasmotaGlobal.uptime > Thermostat[ctr_output].time_ctr_changepoint) ||
			 (Thermostat[ctr_output].temp_measured > Thermostat[ctr_output].temp_target_level_ctr))
			{
			Thermostat[ctr_output].status.command_output = IFACE_OFF;
			Thermostat[ctr_output].time_ctr_checkpoint = TasmotaGlobal.uptime + Thermostat[ctr_output].time_ctr_checktime * 60;
...
void ThermostatWorkAutomaticPI(uint8_t ctr_output)
{
	...
	if (((TasmotaGlobal.uptime >= Thermostat[ctr_output].time_ctr_checkpoint) && (Thermostat[ctr_output].temp_measured <= Thermostat[ctr_output].temp_target_level)) ||
		 (Thermostat[ctr_output].temp_target_level != Thermostat[ctr_output].temp_target_level_ctr)) //||		 (	&&	(Thermostat[ctr_output].status.status_cycle_active == CYCLE_OFF)))
...
void ThermostatWorkAutomaticRampUp(uint8_t ctr_output)
{
	...
	int16_t aux_temp_delta; 						//int16_t temp_delta_rampup;
	bool flag_heating = (Thermostat[ctr_output].status.climate_mode == CLIMATE_HEATING); // Update time in ramp-up as well as delta temp

	time_in_rampup = TasmotaGlobal.uptime - Thermostat[ctr_output].timestamp_rampup_start; //temp_delta_rampup = Thermostat[ctr_output].temp_measured - Thermostat[ctr_output].temp_rampup_start;

	// Init command output status to true
	Thermostat[ctr_output].status.command_output = IFACE_ON; // Update temperature target level for controller
	Thermostat[ctr_output].temp_target_level_ctr = Thermostat[ctr_output].temp_target_level; // If time in ramp-up < max time

	// AND temperature measured < target for heating or > for cooling
	if ((time_in_rampup <= (60 * (uint32_t) Thermostat[ctr_output].time_rampup_max)) && (((Thermostat[ctr_output].temp_measured < Thermostat[ctr_output].temp_target_level) && (flag_heating)) ||
		 ((Thermostat[ctr_output].temp_measured > Thermostat[ctr_output].temp_target_level) && (!flag_heating))))
		{
		}
	else 
		{
		Thermostat[ctr_output].time_ctr_checkpoint = TasmotaGlobal.uptime + Thermostat[ctr_output].time_min_turnoff_action * 60; // Switch Off output
		Thermostat[ctr_output].status.command_output = IFACE_OFF;
		}
}
...
void ThermostatPeakDetector(uint8_t ctr_output)
{
	...
	int16_t peak_avg = 0;
	bool peak_transition = false;					// Update Max/Min Thermostat[ctr_output].temp_abs_max_atune
	...
	if (peak_num < THERMOSTAT_PEAKNUMBER_AUTOTUNE)
		{
		bool flag_heating = (Thermostat[ctr_output].status.climate_mode == CLIMATE_HEATING);
		bool cond_peak_1 = ((Thermostat[ctr_output].temp_measured > Thermostat[ctr_output].temp_peaks_atune[peak_num]) && (flag_heating) ||
			 (Thermostat[ctr_output].temp_measured < Thermostat[ctr_output].temp_peaks_atune[peak_num]) && (!flag_heating));
		bool cond_peak_2 = ((Thermostat[ctr_output].temp_measured < Thermostat[ctr_output].temp_peaks_atune[peak_num]) && (flag_heating) ||
			 (Thermostat[ctr_output].temp_measured > Thermostat[ctr_output].temp_peaks_atune[peak_num]) && (!flag_heating));
		bool cond_gradient_1 = ((Thermostat[ctr_output].temp_measured_gradient > 0) && (flag_heating) || (Thermostat[ctr_output].temp_measured_gradient < 0) && (!flag_heating));
		bool cond_gradient_2 = ((Thermostat[ctr_output].temp_measured_gradient < 0) && (flag_heating) || (Thermostat[ctr_output].temp_measured_gradient > 0) && (!flag_heating)); // If peak number is even (look for max if heating and min if cooling)
		...
		if ((peak_num % 2) == 0)
			...
			if ((cond_peak_2) && (abs(Thermostat[ctr_output].temp_measured - Thermostat[ctr_output].temp_peaks_atune[peak_num]) > Thermostat[ctr_output].temp_band_no_peak_det))
			...
			if ((cond_peak_1) && (abs(Thermostat[ctr_output].temp_measured - Thermostat[ctr_output].temp_peaks_atune[peak_num]) > Thermostat[ctr_output].temp_band_no_peak_det))
				{ // Calculate period
	...
	if ((Thermostat[ctr_output].peak_ctr > 2) && (peak_transition))
		{ //Update peak_num
		peak_num = Thermostat[ctr_output].peak_ctr; // Calculate average value among the last 3 peaks
		peak_avg =
			 (abs(Thermostat[ctr_output].temp_peaks_atune[peak_num - 1] -Thermostat[ctr_output].temp_peaks_atune[peak_num - 2]) +abs(Thermostat[ctr_output].temp_peaks_atune[peak_num - 2] -Thermostat[ctr_output].temp_peaks_atune[peak_num - 3])) / 2;
		if ((20 * (int32_t) peak_avg) < (int32_t) (Thermostat[ctr_output].temp_abs_max_atune - Thermostat[ctr_output].temp_abs_min_atune))
			...
			peak_avg /= peak_num;					// If last period crosses the average value, result valid
...
{
	uint8_t peak_num = Thermostat[ctr_output].peak_ctr; // Calculate the tunning parameters

	// Resolution increased to avoid float operations
	Thermostat[ctr_output].kU_pi_atune =
		 (uint16_t) (100 * ((uint32_t) 400000 * (uint32_t) (Thermostat[ctr_output].dutycycle_step_autotune)) / ((uint32_t) (Thermostat[ctr_output].temp_abs_max_atune - Thermostat[ctr_output].temp_abs_min_atune) * (uint32_t) 314159));
	...
	switch (Thermostat[ctr_output].status.autotune_perf_mode)
		{
		case AUTOTUNE_PERF_FAST: // Calculate kP/Ki autotune
			Thermostat[ctr_output].kP_pi_atune = (4 * Thermostat[ctr_output].kU_pi_atune) / 10;
			break;

		case AUTOTUNE_PERF_NORMAL: // Calculate kP/Ki autotune
			Thermostat[ctr_output].kP_pi_atune = (18 * Thermostat[ctr_output].kU_pi_atune) / 100;
			break;

		case AUTOTUNE_PERF_SLOW: // Calculate kP/Ki autotune
			Thermostat[ctr_output].kP_pi_atune = (13 * Thermostat[ctr_output].kU_pi_atune) / 100;
			break;
		} // Resolution increased to avoid float operations

	Thermostat[ctr_output].kI_pi_atune = (12 * (6000 * Thermostat[ctr_output].kU_pi_atune / Thermostat[ctr_output].pU_pi_atune)) / 10; // Calculate PropBand Autotune
	Thermostat[ctr_output].val_prop_band_atune = 100 / Thermostat[ctr_output].kP_pi_atune; // Calculate Reset Time Autotune
	Thermostat[ctr_output].time_reset_atune =
		 (uint32_t) ((((uint32_t) Thermostat[ctr_output].kP_pi_atune * (uint32_t) Thermostat[ctr_output].time_pi_cycle * 6000)) / (uint32_t) Thermostat[ctr_output].kI_pi_atune);
}


void ThermostatWorkAutomaticPIAutotune(uint8_t ctr_output)
{
	bool flag_heating = (Thermostat[ctr_output].status.climate_mode == CLIMATE_HEATING); // If no timeout of the PI Autotune function

	// AND no change in setpoint
	if ((TasmotaGlobal.uptime < Thermostat[ctr_output].time_ctr_checkpoint) && (Thermostat[ctr_output].temp_target_level_ctr == Thermostat[ctr_output].temp_target_level))
		{
		if (TasmotaGlobal.uptime >= Thermostat[ctr_output].time_ctr_checkpoint)
			{
			Thermostat[ctr_output].temp_target_level_ctr = Thermostat[ctr_output].temp_target_level; // Calculate time_ctr_changepoint
			Thermostat[ctr_output].time_ctr_changepoint =
				 TasmotaGlobal.uptime + (((uint32_t) Thermostat[ctr_output].time_pi_cycle * (uint32_t) Thermostat[ctr_output].dutycycle_step_autotune) / (uint32_t) 100); // Reset cycle active
			Thermostat[ctr_output].status.status_cycle_active = CYCLE_OFF;
...
void ThermostatCtrWork(uint8_t ctr_output)
{
	switch (Thermostat[ctr_output].status.controller_mode)
		...
		case CTR_PI:
			ThermostatRadiator(ctr_output);

			//ThermostatWorkAutomaticPI(ctr_output);
			break; // Ramp-up controller (predictive)
...
void ThermostatWork(uint8_t ctr_output)
{
	switch (Thermostat[ctr_output].status.thermostat_mode)
		...
		case THERMOSTAT_OFF:
			Thermostat[ctr_output].status.command_output = IFACE_OFF;
			break; // State automatic thermostat active following to command target temp.
...
{ // Diagnostic related to the plausibility of the output state
	if ((Thermostat[ctr_output].diag.diagnostic_mode == DIAGNOSTIC_ON) && (Thermostat[ctr_output].diag.output_inconsist_ctr >= THERMOSTAT_TIME_MAX_OUTPUT_INCONSIST))
		{
		Thermostat[ctr_output].status.thermostat_mode = THERMOSTAT_OFF;
...
{
	bool result = false;							// TempVal unit is tenths of degrees celsius

	if ((tempVal >= -1000) && (tempVal <= 1000) && (tempVal >= Thermostat[ctr_output].temp_frost_protect))
		{
...
void ThermostatGetLocalSensor(uint8_t ctr_output)
{
	...
	if (root)
		...
		if ((THERMOSTAT_SENSOR_NUMBER > 1) && (THERMOSTAT_CONTROLLER_OUTPUTS > 1) && (ctr_output < THERMOSTAT_SENSOR_NUMBER))
			{
		...
		if (value_token.isNum())
			...
			if ((value >= -1000) && (value <= 1000) && (Thermostat[ctr_output].status.sensor_type == SENSOR_LOCAL))
				{
				uint32_t timestamp = TasmotaGlobal.uptime; // Calculate temperature gradient if temperature value has changed
	...
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (XdrvMailbox.data_len > 0)
			...
			if ((value >= CLIMATE_HEATING) && (value < CLIMATE_MODES_MAX))
				{
				Thermostat[ctr_output].status.climate_mode = value; // Trigger a restart of the controller
				Thermostat[ctr_output].time_ctr_checkpoint = TasmotaGlobal.uptime;
				}
			}
...
void CmndTempFrostProtectSet(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (XdrvMailbox.data_len > 0)
			...
			if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
				{
				value = (int16_t) ThermostatFahrenheitToCelsius((int32_t) (CharToFloat(XdrvMailbox.data) * 10), 
					TEMP_CONV_ABSOLUTE);
				}
			else 
				{
		...
		if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
			{
			value = ThermostatCelsiusToFahrenheit((int32_t) Thermostat[ctr_output].temp_frost_protect, 
				TEMP_CONV_ABSOLUTE);
			}
		else 
			{
	...
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (XdrvMailbox.data_len > 0)
			...
			if ((value >= CTR_HYBRID) && (value < CTR_MODES_MAX))
				{
				Thermostat[ctr_output].status.controller_mode = value; // Reset controller variables
				Thermostat[ctr_output].timestamp_rampup_start = TasmotaGlobal.uptime;
				Thermostat[ctr_output].temp_rampup_start = Thermostat[ctr_output].temp_measured;
				Thermostat[ctr_output].temp_rampup_meas_gradient = 0;
				Thermostat[ctr_output].time_rampup_deadtime = 0;
				...
				Thermostat[ctr_output].time_ctr_changepoint = 0;
				Thermostat[ctr_output].time_ctr_checkpoint = 0;
				Thermostat[ctr_output].time_integral_pi = ((uint32_t) Thermostat[ctr_output].time_min_action * 60) - 10;
				Thermostat[ctr_output].time_integral_pi_previous = Thermostat[ctr_output].time_integral_pi;
				}
			}
...
void CmndRadiatorTempMeasuredSet(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		{
		uint8_t ctr_output = XdrvMailbox.index - 1;
		...
		if (XdrvMailbox.data_len > 0)
			...
			Thermostat[ctr_output].timestamp_radiator_temp_measured_update = timestamp;
			Thermostat[ctr_output].radiator_temp_measured = value;
			Thermostat[ctr_output].status.ra_sensor_alive = IFACE_ON;
			}
...
void CmndTempMeasuredSet(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (XdrvMailbox.data_len > 0)
			...
			if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
				{
				value = ThermostatFahrenheitToCelsius((int32_t) (CharToFloat(XdrvMailbox.data) * 10), 
					TEMP_CONV_ABSOLUTE);
				}
			else 
				{
			...
			if ((value >= -1000) && (value <= 1000) && (Thermostat[ctr_output].status.sensor_type == SENSOR_MQTT))
				{
				uint32_t timestamp = TasmotaGlobal.uptime; // Calculate temperature gradient if temperature value has changed
				...
				if (Thermostat[ctr_output].status.status_cycle_active == CYCLE_ON)
					{
					Thermostat[ctr_output].temp_measured_accum_error += Thermostat[ctr_output].temp_target_level_ctr - value;
					Thermostat[ctr_output].temp_measured_accum_count += 1;
					}*/
				Thermostat[ctr_output].temp_measured = value;
				Thermostat[ctr_output].timestamp_temp_measured_update = timestamp;
				Thermostat[ctr_output].status.sensor_alive = IFACE_ON;
		...
		if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
			{
			value = ThermostatCelsiusToFahrenheit((int32_t) Thermostat[ctr_output].temp_measured, 
				TEMP_CONV_ABSOLUTE);
			}
		else 
			{
...
void CmndTempTargetSet(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (XdrvMailbox.data_len > 0)
			...
			if ((value >= -1000) && (value <= 1000) && (value >= Thermostat[ctr_output].temp_frost_protect))
				{
		...
		if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
			{
			value = ThermostatCelsiusToFahrenheit((int32_t) Thermostat[ctr_output].temp_target_level, 
				TEMP_CONV_ABSOLUTE);
			}
		else 
			{
...
void CmndTempMeasuredGrdRead(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
			{
			value = ThermostatCelsiusToFahrenheit((int32_t) Thermostat[ctr_output].temp_measured_gradient, 
				TEMP_CONV_RELATIVE);
			}
		else 
			{
...
void CmndTempAntiWindupResetSet(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
			{
			value = ThermostatCelsiusToFahrenheit((int32_t) Thermostat[ctr_output].temp_reset_anti_windup, 
				TEMP_CONV_RELATIVE);
			}
		else 
			{
	...
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (XdrvMailbox.data_len > 0)
			...
			if ((value >= 400) && (value <= 700))
				{
				Thermostat[ctr_output].radiator_temp_max_setting = (int16_t) value;
				}
			}

		ResponseCmndNumber((int) ((uint32_t) Thermostat[ctr_output].radiator_temp_max_setting));
		}
}


void CmndRaTempMinSet(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		{
		uint8_t ctr_output = XdrvMailbox.index - 1;
		...
		if (XdrvMailbox.data_len > 0)
			...
			if ((value >= 300) && (value <= 500))
				{
				Thermostat[ctr_output].radiator_temp_min_setting = (int16_t) value;
				}
			}

		ResponseCmndNumber((int) ((uint32_t) Thermostat[ctr_output].radiator_temp_min_setting));
		}
}
...
void CmndTempRupDeltInSet(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (XdrvMailbox.data_len > 0)
			...
			if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
				{
				value = (uint8_t) ThermostatFahrenheitToCelsius((int32_t) (CharToFloat(XdrvMailbox.data) * 10), 
					TEMP_CONV_RELATIVE);
				}
			else 
				{
		...
		if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
			{
			value = ThermostatCelsiusToFahrenheit((int32_t) Thermostat[ctr_output].temp_rampup_delta_in, 
				TEMP_CONV_RELATIVE);
			}
		else 
			{
...
void CmndTempRupDeltOutSet(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
			{
			value = ThermostatCelsiusToFahrenheit((int32_t) Thermostat[ctr_output].temp_rampup_delta_out, 
				TEMP_CONV_RELATIVE);
			}
		else 
			{
...
void CmndTempRampupPiAccErrSet(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if (XdrvMailbox.data_len > 0)
			...
			if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
				{
				value = (uint16_t) ThermostatFahrenheitToCelsius((int32_t) (CharToFloat(XdrvMailbox.data) * 100), 
					TEMP_CONV_RELATIVE);
				}
			else 
				{
		...
		if (Thermostat[ctr_output].status.temp_format == TEMP_FAHRENHEIT)
			{
			value = ThermostatCelsiusToFahrenheit((int32_t) Thermostat[ctr_output].temp_rampup_pi_acc_error, 
				TEMP_CONV_RELATIVE);
			}
		else 
			{
...
void CmndCtrDutyCycleRead(void)
{
	if ((XdrvMailbox.index > 0) && (XdrvMailbox.index <= THERMOSTAT_CONTROLLER_OUTPUTS))
		...
		if ((Thermostat[ctr_output].status.controller_mode == CTR_PI) || ((Thermostat[ctr_output].status.controller_mode == CTR_HYBRID) &&
			 (Thermostat[ctr_output].status.phase_hybrid_ctr == CTR_HYBRID_PI)))
			{
			value = Thermostat[ctr_output].time_total_pi / Thermostat[ctr_output].time_pi_cycle;
			}
		else if ((Thermostat[ctr_output].status.controller_mode == CTR_RAMP_UP) || ((Thermostat[ctr_output].status.controller_mode == CTR_HYBRID) &&
			 (Thermostat[ctr_output].status.phase_hybrid_ctr == CTR_HYBRID_RAMP_UP)))
			{
...
void CmndThermoRead(void)
{
	...
	if (XdrvMailbox.data_len > 0)
		{
		index = (uint8_t) (CharToFloat(XdrvMailbox.data));
		}

	switch (index)
		{
		case 0:
			value = (int) Thermostat[ctr_output].radiator_temp_max;
			break;

		case 1:
			value = (int) Thermostat[ctr_output].time_ctr_checkpoint;
			break;

		case 2:
			value = (int) Thermostat[ctr_output].time_ctr_changepoint;
			break;

		case 3:
			value = (int) Thermostat[ctr_output].time_proportional_pi;
			break;

		case 4:
			value = (int) Thermostat[ctr_output].time_integral_pi;
			break;

		case 5:
			value = (int) Thermostat[ctr_output].time_integral_pi_previous;
			break;

		case 6:
			value = (int) Thermostat[ctr_output].time_min_action;
			break;

		case 7:
			value = (int) Thermostat[ctr_output].time_min_turnoff_action;
			break;

		case 8:
			value = (int) Thermostat[ctr_output].time_max_action;
			break;

		case 9:
			value = (int) Thermostat[ctr_output].val_prop_band;
			break;

		case 10:
			value = (int) Thermostat[ctr_output].timestamp_radiator_temp_measured_update;
			break;

		case 11:
			value = (int) TasmotaGlobal.uptime;
			break;

		case 12:
			value = (int) Thermostat[ctr_output].status.command_output;
			break;
...
bool Xdrv39(uint32_t function)
{
	...
	switch (function)
		...
		case FUNC_EVERY_SECOND:
			for (ctr_output = 0; ctr_output < THERMOSTAT_CONTROLLER_OUTPUTS; ctr_output++)
				{
				if ((ThermostatHalfMinuteCounter(ctr_output)) && (Thermostat[ctr_output].status.thermostat_mode != THERMOSTAT_OFF))
					{
